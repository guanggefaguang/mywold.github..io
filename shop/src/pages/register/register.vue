<template>
  <div>
    <!-- 公共返回组件 -->
    <com-navigation headerName="注册" :back="false"></com-navigation>
    ...
    <div class="login-main" v-cloak>
      <div class="phone-pwd">
        ...
        <div class="phone">
          <input type="tel" placeholder="请输入验证码" autocomplete="off" v-model="code.value"/>
          <!--<button class="btm">发送验证码</button>-->
        </div>
        <div class="pwd">
          <input type="password" placeholder="请输入密码" autocomplete="off" v-model="password.value"/>
        </div>
      </div>
      <a href="javascript:;" class="com-next" v-on:click="login()" :class="{unable:(mobile.value=='' || password.value=='')}">注册</a>
      <p class="register-back">
        <router-link to="login" class="com-color">已有账号，去登录</router-link>
      </p>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        code : {value:'', success:false, errorInfo: '手机号为空或者格式不正确'},
...
<style scoped>
  ...
  .login-main  .phone {
    ...
    background:url(../../m-images/icon.png) 0 1.108rem no-repeat;
